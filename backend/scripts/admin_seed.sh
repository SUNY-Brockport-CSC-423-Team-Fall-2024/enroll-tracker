...
# Endpoint URL
URL="http://localhost:8002/api/administrators"
...
for i in {1..10}; do
  ...
  if [ "$RESPONSE" -eq 201 ]; then
    echo "Successfully created administrator: $USERNAME"
  else
    echo "Failed to create administrator: $USERNAME - Status Code: $RESPONSE"
  fi
done
