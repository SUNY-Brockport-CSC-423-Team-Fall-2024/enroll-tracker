...
func CreateCourseHandler(s *services.CourseService, w http.ResponseWriter, r *http.Request) {
	...
	course, err := s.CreateCourse(courseCreation)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func GetMajorsAssoicatedWithCourseHandler(s *services.CourseService, w http.ResponseWriter, r *http.Request) {
	//Set CORS
	w.Header().Set("Access-Control-Allow-Origin", "*")
	w.Header().Set("Access-Control-Allow-Methods", "GET")
	w.Header().Set("Access-Control-Allow-Headers", "Content-Type, Authorization")
	...
	courses, err := s.GetMajorsAssociatedWithCourse(courseID, queryParams)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
