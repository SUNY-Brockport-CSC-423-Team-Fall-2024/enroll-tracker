...
export default function TeacherCoursesTable() {
    ...
    const getCourses = async () => {
        try {
            ...
            teachersCourses.map((course) => setCourses([...courses, { content: [course.course_name, `${course.current_enrollment}/${course.max_enrollment}`, new Date(course.last_updated).toLocaleDateString('en-US')], clickable: true, href: `/courses/${course.course_id}`}]));
        } catch(err) {
            console.error(err)
        }
    }
