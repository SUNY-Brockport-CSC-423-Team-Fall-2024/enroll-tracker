  ...
  const fetchAvailableMajors = async () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        setCourse(data);
        setCourseStatus(data.status === "active" ? {label: "Active", value: "active"} : {label: "Inactive", value: "inactive"});
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const resp: AsyncResponse = await updateCourse(course, origCourseMajorIds, courseMajorIds, course.id, courseName, courseDescription, maxEnrollment, numCredits, courseStatus.value);
      ...
      <div className={styles.edit_course_form_container}>
        ...
        <form onSubmit={handleSubmit} id="edit_course_form" className={styles.edit_course_form}>
          <div className={styles.edit_course_form_upper_fields}>
            ...
            <input id="course_name" name="course_name" type="text" defaultValue={course !== null ? course.name : ""} onChange={(e) => handleTextInputChange(setCourseName, e)} required/>
            <label htmlFor="course_description">Course Description</label>
            <textarea id="course_description" form="edit_course_form" name="course_description" defaultValue={course !== null ? course.description : ""} rows={5} maxLength={255} onChange={(e) => handleTextInputChange(setCourseDescription, e)} required/>
            <label htmlFor="course_max_enrollment">Max Enrollment</label>
            <input id="course_max_enrollment" name="course_max_enrollment" type="number" defaultValue={course !== null ? course.max_enrollment : ""} min={0} max={99} onChange={(e) => handleTextInputChange(setMaxEnrollment, e, true)} required/>
            <label htmlFor="course_num_credits">Number of Credits</label>
            <input id="course_num_credits" name="course_num_credits" type="number" defaultValue={course !== null ? course.num_credits : ""} min={1} max={6} onChange={(e) => handleTextInputChange(setNumCredits, e, true)} required/>
          ...
          <div className={styles.edit_course_form_lower_fields_container}>
            <div className={styles.edit_course_form_lower_fields}>
              <input className={componentStyles.secondary_button} type="button" id="course_cancel_edit" name="course_cancel_edit" onClick={() => router.push('/courses')} value="Cancel" />
              <input className={componentStyles.primary_button} id="course_submit" name="course_submit" type="submit" value="Save Changes"/>
            </div>
          </div>
        </form>
